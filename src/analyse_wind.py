...
def bootstrap_median_difference(a: np.ndarray, b: np.ndarray, rng: np.random.Generator) -> tuple[float, float]:
    if len(a) < 10 or len(b) < 10:
        return (np.nan, np.nan)
    values = np.median(rng.choice(a, size=(N_BOOT, len(a)), replace=True), axis=1) - np.median(rng.choice(b, size=(N_BOOT, len(b)), replace=True), axis=1)
...
def rainfall_rate_table(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for keys, group in df.groupby(["Season", "WindGustSpeedGroup"], observed=True):
        season, speed_group = keys
        values = group["RainTomorrow"].dropna().to_numpy(dtype=float)
...
def main(input_path: Path, table_dir: Path, figure_dir: Path) -> None:
    ...
    ax.boxplot(boxes, labels=["0 = no rain", "1 = rain"], patch_artist=True, boxprops={"facecolor": "#8ecae6"}, medianprops={"color": "#023047", "linewidth": 2})
    ...
    for i, season in enumerate(SEASONS):
        ...
        y = subset["rainfall_rate"].to_numpy()
        lower = y - subset["ci_low"].to_numpy()
        upper = subset["ci_high"].to_numpy() - y
        ax.bar(x + (i - 1.5) * width, y, width, yerr=np.vstack([lower, upper]), capsize=3, label=season, color=colors[i])
